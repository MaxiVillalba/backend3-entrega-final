...
import { isAuthenticated, isAdmin } from '../middlewares/auth.js'; // Example middleware
...
// POST: Create an order from the authenticated user's cart
orderRouter.post('/purchase', orderController.createOrderFromCart, isAuthenticated, async (req, res) => {
    try {
        ...
        const cart = await cartDAO.getBy({ user: userId }, { populate: 'products.product' }); // Use populate option
        ...
        for (const item of cart.products) {
            ...
            if (!product || !product.isActive) {
                return res.status(400).json({ status: 'error', message: `Product ${product ? product.name : item.product.toString()} is no longer active or found.` });
            }
...
orderRouter.get('/my-orders', orderController.getUserOrders, isAuthenticated,  async (req, res) => {
    try {
        if (!req.user || !req.user._id) {
            return res.status(401).json({ status: 'error', message: 'User not authenticated.' });
        }
        // Use populate option in get method
        const userOrders = await orderDAO.get({ user: req.user._id }, { populate: { path: 'products.product', select: 'name price' } });
        res.json({ status: 'success', payload: userOrders.orders }); // Note: DAO get method returns an object with 'orders' key
    } catch (error) {
        res.status(500).json({ status: 'error', message: error.message });
    }
});

// GET a specific order by ID (user's own or admin)
orderRouter.get('/:oid', orderController.getOrderById, isAuthenticated, async (req, res) => {
    try {
        if (!req.user || !req.user._id) {
            return res.status(401).json({ status: 'error', message: 'User not authenticated.' });
        }
...
// GET all orders (Admin only, with pagination, filters, sorting)
orderRouter.get('/', isAdmin, orderController.getAllOrders, async (req, res) => {
    try {
        const { page, limit, status, sort, user } = req.query;
        const query = {};
        if (status) query.status = status;
        if (user) query.user = user; // Filter by user ID

        const options = {
            page: parseInt(page) || 1,
            limit: parseInt(limit) || 10,
            sort: sort ? JSON.parse(sort) : null, // Expect sort to be a JSON string like '{"purchaseDate":-1}'
...
orderRouter.put('/:oid', orderController.updateOrderStatus, isAdmin, async (req, res) => {
    try {
        const { oid } = req.params;
